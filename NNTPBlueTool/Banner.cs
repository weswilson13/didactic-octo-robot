...
public class Banner
{
    ...
    {
        string newString = "\n";
        if (Regex.Match(ParentString.TrimStart(),"^[^\\(\\)\\w]").Success) // a bullet should be indented
        {
            newString += "   ";
            if (!string.IsNullOrWhiteSpace(ChildString) && !Regex.Match(ChildString.TrimStart(),"^[^\\(\\)\\w]").Success) { newString += " "; }
        }
    ...
    public static void GovBanner()
    {
        ...
        string newString = "";
        int index;
        int maxLength = (int)((float)0.8 * Global.WindowWidth);
            ...
            if (line.Length > maxLength)
            {
                ...
                while (rem.Length > maxLength)
                {
                    ...
                    if (index != -1)
                    {
                        ...
                        rem = rem.Substring(index + 1).Trim();
                    }
