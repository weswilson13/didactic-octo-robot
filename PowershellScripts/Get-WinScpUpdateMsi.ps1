...
Try {
    ...
    if ($latestVersionAvailable.Version -lt $version) {
        ...
        wget -Uri (Invoke-WebRequest -Uri "https://winscp.net/download/WinSCP-$version.msi" -UseBasicParsing | Select -ExpandProperty Links | Select-String -Pattern 'https.*(?=" class="btn btn-primary btn-lg">Direct download)').Matches[0].Value -OutFile "$WinScpRepo\$($filename.replace('.exe','.msi'))"
